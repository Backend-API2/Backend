...
class DataSubscriptionControllerMatchingTest {
    ...
    void testGetSubscriptionStatus() throws Exception {
        ...
        mockMvc.perform(get("/api/data/subscriptions/status"))
                ...
                .andExpect(jsonPath("$.subscriptions[0]").value("user.user_created"))
                .andExpect(jsonPath("$.subscriptions[1]").value("user.user_updated"))
                .andExpect(jsonPath("$.subscriptions[2]").value("user.user_deactivated"))
                .andExpect(jsonPath("$.subscriptions[3]").value("user.user_rejected"))
                .andExpect(jsonPath("$.subscriptions[4]").value("pago.emitida"))
